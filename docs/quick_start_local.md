# Green DevOps Plugin: Local Quick Start Guide
...
- **Python 3.8+** installed on the Jenkins agent or local machine.
...
You can integrate the Green AI Agent directly into any existing Jenkins pipeline by running the Python optimizer script locally.

### Add the Agent to your Jenkinsfile

Add the following stage to your Jenkinsfile right after your `Checkout` stage. This stage will install dependencies, run the optimizer, fetch the carbon forecast, and output a `dashboard_payload.json` file containing the build commands and deployment strategy.
...
stage('Build Optimizer & Green Scheduling') {
    steps {
        script {
            ...
            // Run the Green AI Agent locally
            sh '''
                # Ensure you have the optimizer script available locally or cloned
                # Example: git clone https://github.com/your-repo/green-agent-lib.git
                
                # Navigate to the optimizer directory if cloned
                # cd green-agent-lib
...
The agent outputs its decisions into the Jenkins environment (or working directory). You can parse this JSON to dynamically control the rest of your pipeline.
...
stage('Selective Build') {
    steps {
        script {
            ...
            // Example: def result = readJSON file: 'dashboard_payload.json'
